...
export function BurgerMenu(props: BurgerMenuProps) {
  ...
  return (
    <>
      <button className={cls([styles.burgerControl, dark && global.dark])} type={'button'} onClick={handleSidebar}>
        {openIcon ??
          <svg xmlns={'http://www.w3.org/2000/svg'} viewBox={'0 0 640 640'} className={styles.icon} ref={el => addAttribution(el)}>
            <path
      ...
      <div className={cls([styles.menuWrapper, alignment === 'left' ? styles.left : styles.right, visibility && styles.transform, dark && global.dark])} onClick={handleSidebar}>
        <button className={styles.burgerControl} type={'button'} onClick={handleSidebar}>
          {closeIcon ??
            <svg xmlns={'http://www.w3.org/2000/svg'} viewBox={'0 0 640 640'} className={styles.icon} ref={el => addAttribution(el)}>
