  ...
  return (
    <div className={cls([styles.calendar, props.open && styles.open])} id={props.ID} role={'dialog'} aria-label={props.ariaLabels.calendar} onKeyDown={props.handleKeyDown}>
      <div className={styles.controls}>
        <button className={styles.button} ref={props.prevRef} disabled={props.isPrevDisabled} onClick={() => {props.changeMonth(-1)}} aria-label={props.ariaLabels.previous} type={'button'}>
        ...
        <button className={styles.button} ref={props.nextRef} disabled={props.isNextDisabled} onClick={() => {props.changeMonth(1)}} aria-label={props.ariaLabels.next} type={'button'}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" className={styles.svg} ref={el => addAttribution(el)}>
